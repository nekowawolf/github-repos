...
export default function StatsHistory() {
  ...
  return (
    <>
      <div className="w-full mt-24 mb-12 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col items-start text-left space-y-2">
          ...
          <p className="text-fill-color/60 text-sm max-w-full sm:max-w-md leading-relaxed">
            Repository statistics and growth over the selected period.
          </p>
        </div>
      ...
      {loading && paginatedData.length === 0 ? (
        ...
        <div className="flex flex-col w-full">
          {paginatedData.length > 0 ? (
            <div className="flex flex-col w-full">
              {paginatedData.map(({ repo, history }, index) => {
                ...
                return (
                  <div
                    ...
                    className={`group flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 py-4 sm:py-5 relative transition-all duration-300 sm:hover:translate-x-1 sm:cursor-pointer cursor-default w-full ${index !== paginatedData.length - 1 ? 'border-b sm:border-b-0 border-[var(--border-divider)] border-opacity-50' : ''}`}
                  >
                    <div
                      onClick={(e) => {
                        if (window.innerWidth < 640) {
                      ...
                      className="flex items-center gap-3 shrink-0 cursor-pointer w-fit sm:w-auto"
                    >
                      ...
                      <div className="flex flex-col min-w-0 flex-1 sm:w-auto">
                        <span className="text-base sm:text-lg font-bold text-fill-color group-hover:text-blue-500 transition-colors duration-300 tracking-tight truncate">
                          {repo.name}
                        </span>
                    ...
                    <div className="flex items-center justify-start sm:justify-end gap-3 sm:gap-4 mt-2 sm:mt-0 w-full sm:w-auto shrink-0">
                      {/* Total Pill */}
                      <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[rgba(var(--fill-color-rgb),0.03)] border border-[var(--border-divider)] group-hover:border-fill-color/20 transition-colors">
                        {statsType === 'stars' ? <FaStar className="w-3.5 h-3.5 text-yellow-500" /> : <FaCodeBranch className="w-3.5 h-3.5 text-blue-500" />}
                        <span className="font-mono text-sm font-semibold text-fill-color/90">{totalVal.toLocaleString()}</span>
                      </div>
                      ...
                      {isAvailable ? (
                        <div className="relative inline-flex">
                          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-colors
                            ${growthVal > 0 ? 'bg-green-500/10 border-green-500/20 text-green-500' :
                              growthVal < 0 ? 'bg-red-500/10 border-red-500/20 text-red-500' :
                                'bg-[rgba(var(--fill-color-rgb),0.03)] border-[var(--border-divider)] text-fill-color/50'}`}
                          >
                            <span className="font-mono text-sm font-bold">
                              {growthVal > 0 ? '+' : ''}{growthVal.toLocaleString()}
                            </span>
                          </div>
                          {/* Attached Period Badge */}
                          <span className="absolute top-0 right-1.5 -translate-y-[40%] translate-x-[30%] text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-fill-color/60 leading-none select-none pointer-events-none">
                            {period === 'today' ? '24H' : period === 'week' ? '7D' : '30D'}
                          </span>
                        </div>
                      ) : (
                        <div className="px-3 py-1.5 rounded-full border border-dashed border-[var(--border-divider)] text-fill-color/40 text-xs font-mono italic">
                          No history
                        </div>
                      )}
                    </div>
